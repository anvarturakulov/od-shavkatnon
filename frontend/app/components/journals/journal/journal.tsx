...
const totals = (item: DocumentModel) => {
    ...
    if (( item.documentType == DocumentType.LeaveMaterial) 
        && item.docTableItems?.length ) {
        ...
        return (
            ...
            documents
                ...
                .filter((item: DocumentModel) => {
                    ...
                    if (journalChechboxs.charges && contentName == DocumentType.LeaveCash && (item.docValues?.isWorker || item.docValues?.isPartner || isFounder(references, item.docValues?.senderId))) return false;
